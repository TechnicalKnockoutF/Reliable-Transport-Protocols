...
struct pkt makePkt(struct msg msgFromAppLayer) {
	...
	newPkt.acknum = nextSeqNum; //Remember to change when you implement bi-directional

//	memcpy(newPkt.payload, msgFromAppLayer.data, sizeof(msgFromAppLayer.data));
	strncpy(newPkt.payload, msgFromAppLayer.data, sizeof(msgFromAppLayer.data));
...
//
	...
	/*perform this part of code only for packets which fall within the window*/
	if (nextSeqNum < base + WINSIZE) {
		...
		A_transport++; //increment count of messages sent out in the network
		...
		/*perform this part of code only when the base of the window is being transmitted*/
		if (base == nextSeqNum) {
			if (timerRunning == 0) { //checks if the timer is running
				starttimer(0, TIMEOUT);
				timerRunning = 1 - timerRunning;
//			printf("Base = nextSeqNum\nTimer started\n");
//			} else {
//				if (timerRunning == 1) {
//					stoptimer(0);
//					starttimer(0, TIMEOUT);
		...
		/*buffer additional packets generated by App layer till the window is available for transmission*/
...
//				timerRunning = 1 - timerRunning;
		...
		/*perform this part of code only for packets which are buffered earlier for transmission when window is available*/
		&& (bufferAPositionStart != (bufferAPositionEnd))) {
//			printf("Sending buffered packets from window to B\n");
			/*Sending buffered packets to B*/
...
void A_timerinterrupt() {
	...
	printf("Retransmitting all UNACKed packets\n");
...
void A_init() {
	...
	/*Initializing timeout on the basis of timeout */
	if (WINSIZE == 10) {
			TIMEOUT = 20.0;
		} else if (WINSIZE == 50) {
			TIMEOUT = 15.0;
...
void B_input(packet)
	...
	if (checkSumAckValue == packet.checksum
			&& expectedSeqNumAtB == packet.seqnum) { //verifies if appropriate sequence number packet is received
...
//		pktAck.payload[0] = '\0';
		...
		tolayer3(1, pktAck); //send ACK in network to A
		printf("ACK sent from B to A\n");

		tolayer5(1, packet.payload); //pass the message to App layer
		B_application++;
		printf("A_application = %d\n", A_application);
		printf("A_transport = %d\n", A_transport);
		printf("B_transport = %d\n", B_transport);
		...
		printf("Packet sent to layer 5 at B\n");

		expectedSeqNumAtB++;
		printf("Next expected Seq Num at B = %d\n", expectedSeqNumAtB);
	} else {
		printf("Corrupt Packet received at B\n");
		/*perform this part of code only if very first packet received is corrupt*/
		if (expectedSeqNumAtB == 1) {
			pktAck.seqnum = 0;
			pktAck.acknum = 0;
...
//			}
			...
			tolayer3(1, pktAck);
//			printf(
//					"First packet received is corrupt\nACK 0 sent from B to A\n");
			/*perform this part of code only when a duplicate packet is received*/
		} else {
			pktAck.seqnum = expectedSeqNumAtB - 1;
			pktAck.acknum = expectedSeqNumAtB - 1;
...
//			pktAck.payload[0] = '\0';
			...
			tolayer3(1, pktAck);
//			printf(
//					"Packet received is corrupt\nACK for previous packet sent from B to A\n");
		}
		printf("ACK sent from B to A\n");
	}
}
