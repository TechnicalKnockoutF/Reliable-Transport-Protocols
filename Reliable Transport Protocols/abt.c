...
int seqNumSentByA;
int ackNumExpectedAtA;
int seqNumExpectedAtB;
int ackNumSentByB;
int onlyOnePacketInTransitFromA;
struct pkt pktAForReXmit; //maintains a copy required for retransmission
struct pkt ackPktAtBForReXmit;//maintains a copy required for retransmission
...
/* Declaring user defined functions start*/
/*
 * Calculates checksum for a given packet
 * @param checkPacket struct that stores the packet whose checksum is to be evaluated or calculated
 * @return checkSumValue calculated checksum value is returned
 */
...
/*
 *
 *Creates a packet with sequence number and checksum values
 *@param msgFromAppLayer message generated by Application Layer is passed to this function
 *@return pkt packet which is to be sent to receiver
 */
struct pkt makePkt(struct msg msgFromAppLayer) {
	...
	seqNumSentByA = 1 - seqNumSentByA; //Toggles value of seqNumA between 0 and 1

	newPkt.seqnum = seqNumSentByA;
	newPkt.acknum = seqNumSentByA; //Remember to change when you implement bi-directional
...
/* Declaring user defined functions end */
...
void A_output(message)
	...
	//printf("Received packet from Layer 5\n");
	...
	if (onlyOnePacketInTransitFromA == 0) {

		pktA = makePkt(message);

		//Maintain a copy of the packet for retransmission
		pktAForReXmit = pktA;
		//printf("Data stored for retransmit\n");

		onlyOnePacketInTransitFromA = 1;
	...
	{
		//printf("Packet dropped as waiting for processing previous packet\n");
	}
}
...
//		struct pkt pktAck; //new struct to hold packet details to be sent to layer 3
	...
	checkSumAck = checkSum(packet);
	//printf("Received packet from Layer 3 at A\n");

	//Verify packet is not corrupted
	if (checkSumAck == packet.checksum) {
		//printf("Expected ACK number at A = %d\n", ackNumExpectedAtA);
		//printf("Received ACK number at A = %d\n", packet.acknum);
		if (ackNumExpectedAtA == packet.acknum) {
			stoptimer(0);
			//printf("ACK received correctly\n");
			ackNumExpectedAtA = 1 - ackNumExpectedAtA;
			//printf("Next expected ACK at A = %d\n", ackNumExpectedAtA);
			onlyOnePacketInTransitFromA = 0;
		} else {
			//do nothing
			//printf("Duplicate ACK received\n");
//			//printf("Expected ACK %d but received ACK %d\n", ackNumExpectedAtA, (1 - ackNumExpectedAtA));
		}
	}
...
void A_timerinterrupt() {
	...
	//printf("-------------------------Code timed out-----------------------\n");
	//packet or ack is lost
	tolayer3(0, pktAForReXmit);
...
void B_input(packet)
	struct pkt packet; {
//	//printf("Data received at B Layer 4\n");
	B_transport++;
	//printf("A_application = %d\n", A_application);
	//printf("A_transport = %d\n", A_transport);
	//printf("B_transport = %d\n", B_transport);
	...
	/*Declaring all variables start*/
	int i;
	int checkSumForPktFromA = 0;
	struct pkt pktAck; //new struct to hold packet details to be sent to layer 3
	/*Declaring all variables end*/

	checkSumForPktFromA = checkSum(packet);

	//Verify packet is not corrupted
	if (checkSumForPktFromA == packet.checksum) {
		//printf("Expected sequence number at B = %d\n", seqNumExpectedAtB);
		//printf("Received sequence number at B = %d\n", packet.seqnum);
		if (packet.seqnum == seqNumExpectedAtB) {
			...
			pktAck.seqnum = packet.seqnum; //Remember to change when you implement bi-directional
			pktAck.acknum = packet.seqnum;
			//modify this when you implement bi-directional
//			for (i = 0; i < 20; i++) {
//				pktAck.payload[i] = '\0';
//			}
			...
			tolayer3(1, pktAck);
			ackPktAtBForReXmit = pktAck;

			seqNumExpectedAtB = 1 - seqNumExpectedAtB;
			//printf("Next sequence number expected at B = %d\n", seqNumExpectedAtB);
		} else //if the packet sequence is not the expected sequence --> Retransmit old ACK
		{
			tolayer3(1, ackPktAtBForReXmit);
			//printf("ACK for previous packet resent\n");
		}
	}
...
void B_init() {
	ackPktAtBForReXmit.acknum = 0;
	ackPktAtBForReXmit.seqnum = 0;
	ackPktAtBForReXmit.payload[0] = '\0';
	ackPktAtBForReXmit.checksum = checkSum(ackPktAtBForReXmit);
}
